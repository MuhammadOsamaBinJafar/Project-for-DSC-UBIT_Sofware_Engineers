...
switch = None
switch2 = None
# ================== Defining Button on press function =============== #
...
#==================defining clear screen on press

def btnc_clicked(*args):
    disp.delete(0, END)
    disp.insert(0, '0')

#==================defining scientific functions
...
#=================defining delete fn to delete last entry only
def del_clicked():
    pos = len(disp.get())
...
#==================defining degree/radians converter
def conv_clicked():
    global switch
    if switch is None:
        switch = True
...
#===============defining equal by evaluate to get final result
...
#============ Label as display screen

data = StringVar()

disp = Entry(root, font="Verdana 20", fg="black", bg="lightgreen", bd=0, justify=RIGHT, insertbackground="#abbab1", cursor="arrow")
#============== binding keyboard to graphical user interface
...
def Developer_clicked():

    global switch2
    if switch2 is None:
        switch2 = True
        Developer['text'] = "MUHAMMAD OSAMA BIN JAFAR"
    else:
        switch2 = None
...
Developer = Button(btnrow5, text="DEVELOPER", font="Segoe 23", relief=GROOVE, bd=0, command=Developer_clicked, fg="yellow", bg="#333333")
Developer.pack(side=LEFT, expand=TRUE, fill=BOTH)
mainloop()
